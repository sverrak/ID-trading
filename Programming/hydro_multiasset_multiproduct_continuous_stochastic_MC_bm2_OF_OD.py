...
#		for i in range(number_of_trading_stages):
#			for j in range(number_of_trading_stages):
#				model.addConstr(transaction_prices[dp][s][i][j] == sum(price_levels[dp][s][p] * transaction_volumes[dp][s][i][j][p] for p in range(number_of_price_levels)), "set transaction price"+str(i)+str(j))
...
for dp in range(number_of_dps):
	for s in range(len(scenario_probabilities)):
		for i in range(number_of_trading_stages):
			for p in range(number_of_price_levels):
				model.addConstr(transaction_volumes[dp][s][i][i][p] <= volume_levels[dp][i][p][s], "no_transaction_before_placement")
