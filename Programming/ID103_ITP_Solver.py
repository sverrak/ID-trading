...
import sys
sys.path.append('C:/Users/sverrak/Orderbooks/ComXervOrderbooks_2016_11_16-2016_11_30/Output')
...
class ITP_Solver(object):
    ...
    def __init__(self, generate_scenarios=False, generate_random_variables=False, printing_output=False, output_solutions=False, parameter_file_name="Data/ID101_hydro_multiasset_multiproduct_continuous_stochastic_MC_parameters_bm_OD.txt"):
        ...
        self.generate_transition_matrix                 = "Empirically"
        self.transition_matrix_directory                = 'C:/Users/sverrak/Orderbooks/ComXervOrderbooks_2016_11_16-2016_11_30/Output/'
    ...
    def create_pdf(self, transitions):
        self.transitions_pdf = [[0.0 for x in range(self.size_of_price_range)] for y in range(self.size_of_price_range)]        # Transition PDFs. Normalized version of self.transitions
        for i in range(self.size_of_price_range):
            normalizing_factor = sum(transitions[i])
            for j in range(self.size_of_price_range):
                try:
    ...
    def setup_transition_matrix(self):
        # Parameter setting
        price_increment = 0.1
        self.size_of_price_range = int(self.price_level_bounds[1] - self.price_level_bounds[0])
        transitions = [[0.0 for x in range(self.size_of_price_range * price_increment + 1)] for y in range(self.size_of_price_range * price_increment + 1)]            # Array to be filled with unnormalized "probabilities"
        
        # Depending on transition generation mode, create an unnormalized pdf
        if(self.generate_transition_matrix == "Dummy"):
            
            for i in range(self.size_of_price_range):
                for j in range(self.size_of_price_range):
                    transitions[i][j] = numpy.exp(-numpy.abs(i-j)) # Dummy unnormalized pdf
            
        elif(self.generate_transition_matrix == "Empirically"):
            
            load_matrix = False
            save_matrix = True
            
            if(load_matrix == True):
                size = int((self.price_level_bounds[1] - self.price_level_bounds[0]) * 10) + 2
                transitions = self.load_transition_matrix(size)
            else:
                date_range = ["2016-09-01", "2017-02-28"]
                price_range = [int(i) for i in self.price_level_bounds]
                matrix_learner = TransitionMatrixLearner(date_range, price_range, price_increment, self.transition_matrix_directory)
                matrix_learner.learn_matrix()
                matrix_learner.normalize_matrix()
                transitions = matrix_learner.get_matrix()
                
                if(save_matrix == True):
                    self.save_transition_matrix(transitions)
                
            
        # Normalize the unnormalized pdf
        self.create_pdf(transitions)
        
        # Create the cdfs
        self.create_cdf()
            
    def load_transition_matrix(self, size):
        file_name = "transitions" + str(size) + ".csv"
        transitions = []
        
        with open(file_name, 'r') as csvfile:
            reader = csv.reader(csvfile, delimiter=' ', quotechar='|')
            for row in (reader):
                transitions.append([float(x) for x in list(row[0].split(","))])
            
        return transitions
    
    def save_transition_matrix(self, transitions):
        file_name = "transitions" + str(len(transitions)) + ".csv"
        print(transitions)
                ...
                for t in range(self.number_of_trading_stages):
                    for p in range(self.number_of_price_levels + self.number_of_cpr_price_levels):
                        ...
                        # Set the cpr to 1. Otherwise, set it to 0. 
                            ...
                            # If the previous price level got a CPR == 1.0, we can assess setting CPR to 1.0. Otherwise, it must be forced to 0.0. 
                            if(float(self.cpr[dp][t][p-1]) == 1.0):
