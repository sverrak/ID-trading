...
parameter_file 							= "Data/hydro_multiasset_multiproduct_continuous_stochastic_MC_parameters.txt"
variables_file_name						= "variables.xlsx"
printing_mode							= False
...
def read_parameters(parameter_file):
	...
	index_of_transaction_costs = index_of_scenario_probabilities + 2 + 4 * number_of_dps # If necessary, modify this
	...
	return number_of_production_units, volume_options, min_q, max_q, scenario_probabilities, scenarios, transaction_cost, production_cost, production_capacities, cpr, number_of_dps, initial_storage, storage_bounds, scenario_inflows, scenario_production_capacities, scenario_production_costs
...
number_of_production_units, volume_options, min_q, max_q, scenario_probabilities, scenarios, transaction_cost, production_cost, production_capacities, cpr, number_of_dps, initial_storage, storage_bounds, scenario_inflows, scenario_production_capacities, scenario_production_costs = read_parameters(parameter_file)
#print(scenario_probabilities)
test_parameters()
...
transaction_revenues 					= [[[[0 for k in range(number_of_trading_stages)] for j in range(number_of_trading_stages)] for i in range(len(scenario_probabilities))] for dp in range(number_of_dps)]   # Total transaction volume
...
for dp in range(number_of_dps):
	for s in range(len(scenario_probabilities)):
		for i in range(number_of_trading_stages):
			bid_volume[dp][s][i] = model.addVar(vtype=GRB.CONTINUOUS, lb=-40.0,                              					# Transaction price
...
for dp in range(number_of_dps):
	for s in range(len(scenario_probabilities)):
		for i in range(number_of_trading_stages):
			for j in range(number_of_trading_stages):
				transaction_revenues[dp][s][i][j] = model.addVar(vtype=GRB.CONTINUOUS,          	                    # Transaction price
...
for dp in range(number_of_dps):
	for s in range(len(scenario_probabilities)):
		for i in range(number_of_trading_stages):
			for j in range(number_of_trading_stages):
				model.addConstr(transaction_revenues[dp][s][i][j] == transaction_volumes[dp][s][i][j] * transaction_prices[dp][s][i][j], "bp=pc"+str(i)+str(j))
