...
class TimeslotState(object):
    ...
    def __init__(self, timeslot, base_price, spread=0, buy_order_depth=0, sell_order_depth=0, best_buy_price=0, best_sell_price=0,low_transaction_price=0,high_transaction_price=0,bvs=[],bps=[],svs=[],sps=[],base_price_equal_mid_price=False):
        super(TimeslotState, self).__init__()

        # Model parameters
        self.timeslot = timeslot
        self.base_price = base_price if base_price_equal_mid_price == False else 0.5 * (best_buy_price + best_sell_price)
        self.spread = spread
