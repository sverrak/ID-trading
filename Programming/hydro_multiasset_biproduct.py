...
parameter_file = "Data/hydro_multiasset_biproduct_parameters.txt"
...
def read_parameters(parameter_file):
	...
	print(index_of_scenario_probabilities + 2 + 3 * number_of_dps)
...
bid_prices = [[[model.addVar(name="bid_price_"+str(i)+"_"+str(j)) for j in range(number_of_trading_stages)] for i in range(len(scenario_probabilities))] for dp in range(number_of_dps)]
...
production_quantities = [[[model.addVar(vtype=GRB.CONTINUOUS,lb=min_q,ub=max_q, name="production_quantity_"+str(dp)+"_"+str(s)+"_"+str(j)) for j in range(len(production_capacities))] for s in range(len(scenario_probabilities))] for dp in range(number_of_dps)]    # Production quantity
storage_volume = [[model.addVar(name="storage_volume_"+str(dp)+"_"+str(s)) for s in range(len(scenario_probabilities))] for dp in range(number_of_dps)]
	...
	for s in range(len(scenario_probabilities)):
		for i in range(number_of_trading_stages):
			bid_volume[dp][s][i] = model.addVar(vtype=GRB.CONTINUOUS,                              					# Transaction price
		                                    name=str("b_v_"+str(dp)+"_"+str(s)+"_"+str(i)))
for dp in range(number_of_dps):
	for s in range(len(scenario_probabilities)):
		for i in range(number_of_trading_stages):
...
for dp in range(number_of_dps):
	for s in range(len(scenario_probabilities)):
		print("Inflow_"+str(dp)+"_"+str(s)+ " " + str(scenario_inflows[dp][s]))
