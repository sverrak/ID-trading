...
class ITP_Solver(object):
    ...
    def instantiate_datastructures(self, from_reset=False):
                        ...
                        if (False):#(s > 0 and t == 0):
                            ...
                            if (p == 0):
                                if(t <= anticipativity_threshold):
                                    ...
                                    self.price_levels_negative[dp][t][p][s]         = int(self.price_level_bounds[0]     - (self.price_level_bounds[1]         - self.price_level_bounds[0])     * random_variables[s_index][2])
                                    ...
                                    self.price_levels_negative[dp][t][p][s]         = int(self.price_level_bounds[0]     - (self.price_level_bounds[1]         - self.price_level_bounds[0])     * random_variables[s_index][5])
                                    self.volume_levels_negative[dp][t][p][s]        = self.volume_levels[dp][t][p][s]
                                ...
                                if(t <= anticipativity_threshold):
                                    ...
                                    self.volume_levels[dp][t][p][s]        = int(self.volume_level_bounds[0]     + (self.volume_level_bounds[1]         - self.volume_level_bounds[0])     * random_variables[s_index][1])
                                    self.price_levels_negative[dp][t][p][s]         = int(self.price_level_bounds[0]     - (self.price_levels_negative[dp][t][p-1][s]    - self.price_level_bounds[0])     * random_variables[s_index][0])
                                    self.volume_levels_negative[dp][t][p][s]        = self.volume_levels[dp][t][p][s] 
                                else:
                                    self.price_levels[dp][t][p][s]         = int(self.price_level_bounds[0]     + (self.price_level_bounds[1]    - self.price_levels[dp][t][p-1][s])     * random_variables[s_index][0])
                                    self.volume_levels[dp][t][p][s]        = int(self.volume_level_bounds[0]     + (self.volume_level_bounds[1]         - self.volume_level_bounds[0])     * random_variables[s_index][1])
        ...
        for pu in range(self.number_of_production_units):
            for s in range(self.number_of_scenarios):
                if(self.stochastic_production_parameters):
                    ...
                    # To do: insert non-arbitrary value here
                    self.scenario_production_capacities[pu][s] = 5100000
            ...
            for t in range(self.number_of_trading_stages):
                for p in range(self.number_of_price_levels+self.number_of_cpr_price_levels):
                    ...
                    # Set the cpr to 1. Otherwise, set it to 0. 
                        ...
                        #print(dp,t,p, price_level_probability, sum(self.cpr[dp][t][p][s] for s in range(self.number_of_scenarios)))

        ### Extend the volume levels
        ...
        if(from_reset == False):
            print("PL:", self.number_of_price_levels)
            for dp in range(self.number_of_dps):
                for t in range(self.number_of_trading_stages):
    ...
    def setup_non_anticipativity_constraints(self):
        ...
        if(self.asynchronous_gcs):
            ...
            for dp in range(self.number_of_dps):
                ...
                print("\n At stage",self.time_of_first_gc + self.stages_per_hour * dp,"there are",len(NA_set),"scenario keys")
                
                # For each of these NA_sets, we create NACs
                for j,key in enumerate(list(NA_set.keys())):
                    # If there are more than one scenario having a certain scenario key, we must create NA constraints!
                    if(len(NA_set[key]) > 1):
                        print("\t For key",j,"there are",len(NA_set[key]),"scenarios")
                            
                        for i,s in enumerate(list(NA_set[key])[0:-2]): # Not consid
                            for pu in range(self.number_of_production_units):
    ...
    def write_variables_to_file(self):
        ...
        for dp in range(self.number_of_dps):
            for s in range(self.number_of_scenarios):
                ...
                counter += 1
        for t in range(self.number_of_trading_stages):
            for j,k in enumerate(list(self.non_anticipativity_sets[t].keys())):
                sheet.write(counter, col_counter, "NA_set_"+str(t)+"_"+str(j)+" "+str(self.non_anticipativity_sets[t][k]))
                #sheet.write(counter, col_counter+i+1, v)
                if(counter > 500000):
                    col_counter += 10
                    counter = 1
                counter += 1
